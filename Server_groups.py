...
def handle_file_transfer(connection, args):
    try:
        ...
        if recipient_socket:
            recipient_socket.send(f"file {filename} {filesize}".encode())
            if transfer_file_data(connection, recipient_socket, filesize):
                connection.send("RECEIVED".encode())  # Confirmation to the sender
            else:
        ...
        with db_connection() as conn:
            ...
            while True:
                ...
                # Store all messages in the database
                    ...
                    friend_username, message_content = args[0], ' '.join(args[1:])
                    formatted_message = format_message_with_timestamp(username, message_content)
                
                    try:
                        friend_connection = get_online_user(friend_username)
                        if friend_connection:
                            # Send message to the friend and mark it as delivered
                            friend_connection.send(formatted_message.encode("utf-8"))
                            cursor.execute("INSERT INTO messages (sender, receiver, message, delivered) VALUES (?, ?, ?, 1)", (username, friend_username, message_content))
                        else:
                            # Store the message if the friend is offline
                            cursor.execute("INSERT INTO messages (sender, receiver, message, delivered) VALUES (?, ?, ?, 0)", (username, friend_username, message_content))
                        
                        conn.commit()  # Commit changes to the database
                    except Exception as e:
                        print(f"Error sending message to {friend_username}: {e}")
                    ...
                    deliver_undelivered_messages(cursor, username, connection)  # Deliver undelivered messages
                elif command == "logout":
                    if username:
                        remove_online_user(username)  # Update the user's status to offline
                    connection.send("Logout successful.".encode())  # Send acknowledgment
                 
                
                elif command == "add_to_group":
                    group_id, member_username = args
                    try:
                    ...
                    group_name = ' '.join(args)  # The entire argument string is the group name
                    try:
                        cursor.execute('INSERT INTO groups (name, created_by) VALUES (?, ?)', (group_name, username))
                        group_id = cursor.lastrowid
                        # Add the group creator automatically to the group members
                        cursor.execute('INSERT INTO group_members (group_id, username) VALUES (?, ?)', (group_id, username))
                        conn.commit()
                        connection.send("Group created successfully.".encode())
                    except Exception as e:
                    ...
                    try:
                        cursor.execute('SELECT g.id, g.name FROM groups g JOIN group_members gm ON g.id = gm.group_id WHERE gm.username = ?', (username,))
                        ...
                        connection.send(f"Error listing groups: {str(e)}".encode())
...
while True:
    ...
    threading.Thread(target=client_thread, args=(connectionSocket, addr)).start() # RESPONSIBLE FOR MULTI-THREADING , in terms of creating a new thread for each new client which connects to the server !
